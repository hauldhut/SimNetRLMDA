...
# Step 1: Load the miR similarity network
def load_miR_network(miR_sim_file="../Data/miRNANetS.txt"):
    print(miR_sim_file)
    
    print("Loading miR similarity network...")
    miR_sim_df = pd.read_csv(miR_sim_file, sep='\t', header=None, names=['miR1', 'sim', 'miR2'])
    miRs = set(miR_sim_df['miR1']).union(set(miR_sim_df['miR2']))
    
    miR_G = nx.Graph()
    for miR in tqdm(miRs, desc="Adding miR nodes"):
        miR_G.add_node(miR, type="miR")
    
    for _, row in tqdm(miR_sim_df.iterrows(), total=len(miR_sim_df), desc="Adding miR-miR edges"):
        miR_G.add_edge(row['miR1'], row['miR2'], weight=float(row['sim']))
    
    return miR_G, list(miRs)
...
def save_embeddings_miR(miR_embeddings, miR_G, output_file):
    print("Saving embeddings...")
    data = []
    
    for node, emb in miR_embeddings.items():
        node_type = miR_G.nodes[node]["type"]
        row = [node, node_type] + emb.tolist()
        data.append(row)
    
    embedding_size = len(next(iter(miR_embeddings.values())))
    columns = ['node_id', 'type'] + [f'dim_{i+1}' for i in range(embedding_size)]
    df = pd.DataFrame(data, columns=columns)
    df.to_csv(output_file, index=False)
    print(f"miR Embeddings saved to {output_file}")
...
# Main function
def main(embedding_size=128, epochs=50, miR_sim_file="../Data/miRNANetS.txt"):
    # Load miR network (only once)
    miR_base_name = os.path.splitext(os.path.basename(miR_sim_file))[0]
    miR_output_file = f"../Results/{miR_base_name}_{emb_method}_d_{embedding_size}_e_{epochs}.csv" 
    if os.path.exists(miR_output_file):
        print(f"File {miR_output_file} is existing!")
    else: 
        miR_G, miRs = load_miR_network(miR_sim_file)
        print("Converting miR graph to PyTorch Geometric format...")
        miR_data, miR_node_to_idx = graph_to_pyg_data(miR_G, embedding_size)
        print("Computing miR embeddings...")
        miR_embeddings = train_gat(miR_data, embedding_size=embedding_size, epochs=epochs, node_type="miRs")
        miR_embeddings_dict = extract_embeddings(miR_embeddings, miRs, miR_node_to_idx, "miRs")
        # Save miR embeddings
        save_embeddings_miR(miR_embeddings_dict, miR_G, miR_output_file)

        # Print sample embeddings
        print(f"\nSample miR embeddings for {miR_output_file}:")
        for node, emb in list(miR_embeddings_dict.items())[:5]:
            print(f"miR Node: {node}, Embedding: {emb[:5]}...")
    
    
# Execute and get embeddings
if __name__ == "__main__":
    miR_sim_file = "../Data/miRNANetS.txt"#miRNANetW/miRNANetS/miRNANetB/miRNANetSB/miRNANetWSB
    print(f"miR_sim_file: {miR_sim_file}")
